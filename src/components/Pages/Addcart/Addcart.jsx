...
const Addcart = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        const response = await axios.get(`https://restaurent-s.vercel.app/cart?userEmail=${user.email}`,{withCredentials:true});
  ...
  const handleDelete = async (itemId) => {
    try {
      ...
      if (result.isConfirmed) {
        const response = await fetch(`https://restaurent-s.vercel.app/cart/${itemId}`, {
          method: 'DELETE',
        });
